...
def get_edge_index(surface):
    ...
    T = np.concatenate(
        [np.expand_dims(np.concatenate([triangles[:, 0], triangles[:, 0], triangles[:, 1]], axis=0), axis=1),
         np.expand_dims(np.concatenate([triangles[:, 1], triangles[:, 2], triangles[:, 2]], axis=0), axis=1)], axis=1)
    E = np.concatenate([np.max(T, axis=1, keepdims=True), np.min(T, axis=1, keepdims=True)], axis=1)
    E = np.unique(E, axis=0)
    E_rev = np.concatenate([np.max(E, axis=1, keepdims=True), np.min(E, axis=1, keepdims=True)], axis=1)
...
def create_patient_dict(path, savepath):
    """
    Creates dictionary with patients as key and list of segments for each patient as item
...
def create_mesh_data(surface, div=False, coord=False):
    """
        Creates data for the mesh where each node contains the 30 WSS magnitude and other features
    ...
    for t in range(30):
        wssArray_t = vtk_to_numpy(surface.GetPointData().GetArray('WSS_' + '{0:03}'.format(t + 1)))
        X = np.expand_dims(wssArray_t, axis=1)
        if div:
            div_t = vtk_to_numpy(surface.GetPointData().GetArray('DIV_' + '{0:03}'.format(t + 1)))
...
def create_data(name, k_neigh, rot_angles, path, label_path):
    savepath = os.path.join('data/', name)
    ...
    for pt in data_dict.keys():
        for pt_seg in data_dict[pt]:
            ...
            cur_path = os.path.join(savepath, pt_seg + '.pt') # file to save in
            surface = read_poly_data(path + '/' + pt_seg + '_WSSMag.vtp') # read data
            pos = torch.from_numpy(vtk_to_numpy(surface.GetPoints().GetData())).type(torch.FloatTensor) # get points of surface
            num_nodes = len(pos)
            edges = get_edge_index(surface)
            edge_index = torch.from_numpy(edges.transpose()).type(torch.LongTensor)
            ...
            if name == 'CoordToCnc':
                ...
                wss_mag = np.mean(create_mesh_data(surface, div=False, coord=False), axis=1).reshape(-1, 1)
                one_culprits = np.ones((num_nodes, 1)) * culprit
                y = torch.from_numpy(np.concatenate((wss_mag, one_culprits), axis=1)).type(torch.FloatTensor)
                segment_data = vtk_to_numpy(surface.GetPoints().GetData())
            ...
            data = Data(x=segment_data, edge_index=edge_index, y=y, pos=pos, segment=NAME_TO_INT[seg])
            torch.save(data, cur_path)

            # k_neighbours data aug:
            if k_neigh != 0:
                edges = create_knn_data(surface, k_neigh)
                data = Data(x=segment_data, edge_index=edges, y=y, pos=pos, segment=NAME_TO_INT[seg])
                torch.save(data, os.path.join(savepath, pt_seg + '1.pt'))

            # rotation_data aug
            for angle in rot_angles:
                mat = torch.tensor([[1, 0, 0],
                                     [0, np.cos(math.radians(angle)), -np.sin(math.radians(angle))],
                                     [0, np.sin(math.radians(angle)), np.cos(math.radians(angle))]]).float()
                segment_data_rot = torch.transpose(mat@torch.transpose(segment_data, 0,1), 0,1)
                data = Data(x=segment_data_rot, edge_index=edge_index, y=y, pos=pos, segment=NAME_TO_INT[seg])
                torch.save(data, os.path.join(savepath, pt_seg + '_rot' + f'{angle:03d}' + '.pt'))
